...
import id.com.diaryme.meliasepti.jasatatabusana.fragments.BerandaFragment
import id.com.diaryme.meliasepti.jasatatabusana.fragments.JasaFragment
import id.com.diaryme.meliasepti.jasatatabusana.fragments.ProfileFragment
import id.com.diaryme.meliasepti.jasatatabusana.helpers.Config
import id.com.diaryme.meliasepti.jasatatabusana.helpers.SessionHandler
    ...
    private fun openFragment(fragment_id : Int) {
        when (fragment_id) {
            R.id.nav_beranda -> {
                ...
                supportFragmentManager.beginTransaction().replace(R.id.frame_layout, berandaFragment).setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN).commit()
            }
            R.id.nav_jasa_pengguna -> {
                jasaFragment = JasaFragment()
                supportFragmentManager.beginTransaction().replace(R.id.frame_layout, jasaFragment).setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN).commit()
            }
            R.id.nav_profile -> {
                profileFragment = ProfileFragment()
                supportFragmentManager.beginTransaction().replace(R.id.frame_layout, profileFragment).setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN).commit()
            }
            R.id.nav_logout -> {
                val builder = AlertDialog.Builder(this)
                builder.setTitle("Keluar akun")
                builder.setMessage("Apakah kamu yakin ini keluar dari akun ?")
